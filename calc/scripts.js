    ...
    for (const key in ghost) {
        if (key !== 'id' && key !== 'rank_modifier') {
            ...
            if (key === 'name' || key === 'rarity_rank' || key === 'speed' || key === 'stamina' || key === 'smarts' || key === 'luck' || key === 'farming_length') {
                statsLeftHTML += `<p><strong>${imgHtml}<strong>${prettyKey}:</strong><span>${statValue}</span></p>`;
            } else {
                statsRightHTML += `<p><strong>${imgHtml}<strong>${prettyKey}:</strong><span>${statValue}</span></p>`;
            }
        }
    }
